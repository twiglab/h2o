...
func newConnector(p *Plugin, config *ConnectionConfig) (*connector, error) {
	...
	client, err := newS7Client(config)
	if err != nil {
		return nil, fmt.Errorf("create s7 client error: %w", err)
	}
	conn.client = client
	return conn, nil
}
...
func (c *connector) createNodeGroups(model config.DeviceModel, device config.Device) {
	for _, point := range model.DevicePoints {
		...
		if nodeCfg.Area == "" {
			driverbox.Log().Warn("point has no area", zap.String("point", point.Name()))
			continue
		}
...
func (c *connector) collectData() {
	...
	if c.client == nil {
		driverbox.Log().Error("s7 client is nil")
		return
	}
	values, err := c.client.ReadNodes(c.getNodes())
	if err != nil {
		driverbox.Log().Error("read s7 nodes error", zap.Error(err))
		return
	}
...
func (c *connector) getNodes() []*NodeConfig {
...
func (c *connector) processReadValues(values map[string]interface{}) plugin.DeviceData {
	...
	for pointName, node := range c.nodes {
		if value, ok := values[pointName]; ok {
...
func (c *connector) Encode(deviceId string, mode plugin.EncodeMode, values ...plugin.PointData) (res interface{}, err error) {
	if mode == plugin.ReadMode {
		nodes := make([]*NodeConfig, 0, len(values))
		for _, v := range values {
			if node, ok := c.nodes[v.PointName]; ok {
				nodes = append(nodes, node)
			}
		}
		return &ReadRequest{Nodes: nodes}, nil
	}
...
func (c *connector) Send(data interface{}) (err error) {
	...
	defer c.mutex.Unlock()
	if c.client == nil {
		return errors.New("s7 client is nil")
	}
	...
	case []*WriteRequest:
		for _, wr := range req {
			if err := c.client.WriteNode(wr.Node, wr.Value); err != nil {
				driverbox.Log().Error("write node error", zap.String("point", wr.Node.PointName), zap.Error(err))
			}
		}
		return nil
	default:
