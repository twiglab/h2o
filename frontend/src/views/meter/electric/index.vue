  ...
  <div class="page-container">
    ...
    <div class="table-container">
      <div class="table-toolbar">
        <span class="table-title">电表列表</span>
        <div>
          <el-button type="primary" :icon="Plus" v-permission="'meter:electric:create'" @click="handleAdd">
            新增
          </el-button>
          <el-button type="success" :icon="Download" v-permission="'meter:electric:export'" @click="handleExport">
            导出
          </el-button>
        </div>
      </div>
      ...
      <el-table v-loading="loading" :data="tableData" stripe border>
        ...
        <el-table-column label="关联店铺" min-width="100" show-overflow-tooltip>
          <template #default="{ row }">
            {{ row.shop_name || '-' }}
          </template>
        </el-table-column>
        <el-table-column prop="comm_addr" label="通信地址" width="120" />
        <el-table-column prop="current_reading" label="当前读数" width="110" align="right">
          <template #default="{ row }">
            <span class="text-money">{{ parseFloat(row.current_reading)?.toFixed(2) || '0.00' }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="multiplier" label="倍率" width="80" align="center" />
        <el-table-column prop="online_status" label="在线状态" width="90" align="center">
          <template #default="{ row }">
            <template v-if="row.online_status != null">
              <el-tag :type="row.online_status === 1 ? 'success' : 'danger'" size="small">
        ...
        <el-table-column prop="last_collect_at" label="最后采集时间" width="170">
          <template #default="{ row }">
            {{ formatDateTime(row.last_collect_at) || '-' }}
          </template>
        </el-table-column>
        ...
        <el-table-column label="操作" width="150" fixed="right">
          <template #default="{ row }">
            <el-button type="primary" text size="small" v-permission="'meter:electric:update'" @click="handleEdit(row)">
              编辑
            </el-button>
            <el-button type="danger" text size="small" v-permission="'meter:electric:delete'" @click="handleDelete(row)">
              删除
            </el-button>
          </template>
        </el-table-column>
    ...
    <el-dialog
      ...
      :title="dialogType === 'add' ? '新增电表' : '编辑电表'"
      width="650px"
      destroy-on-close
    >
...
import type { Merchant, Shop, ElectricMeter } from '@/types/business'
import type { Account } from '@/types/finance'

const loading = ref(false)
const tableData = ref<ElectricMeter[]>([])
const total = ref(0)
...
async function fetchList() {
  loading.value = true
  try {
    const res = await meterApi.getElectricMeterList(queryParams)
...
async function fetchRateList() {
  try {
    const res = await rateApi.getRateList({ page: 1, pageSize: 1000, status: 1 })
...
// 编辑
function handleEdit(row: ElectricMeter) {
    ...
    if (valid) {
      ...
      try {
        if (dialogType.value === 'add') {
          await meterApi.createElectricMeter(formData.value)
        } else {
          await meterApi.updateElectricMeter(formData.value.id!, formData.value)
        }
        ElMessage.success(dialogType.value === 'add' ? '新增成功' : '编辑成功')
        dialogVisible.value = false
        fetchList()
      } catch (error) {
        console.error('保存电表失败:', error)
      } finally {
...
// 删除
async function handleDelete(row: ElectricMeter) {
  await ElMessageBox.confirm(`确定要删除电表"${row.meterNo}"吗？`, '提示', { type: 'warning' })
  try {
    await meterApi.deleteElectricMeter(row.id)
