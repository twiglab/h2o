...
// The ViewRecordFunc type is an adapter to allow the use of ordinary
// function as ViewRecord mutator.
type ViewRecordFunc func(context.Context, *ent.ViewRecordMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f ViewRecordFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	if mv, ok := m.(*ent.ViewRecordMutation); ok {
		return f(ctx, mv)
	}
	return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.ViewRecordMutation", m)
}
